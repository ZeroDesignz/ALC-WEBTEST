...
export const Balancebar = (props) => {
  ...
  useEffect(() => {
    const getBalance = async () => {
      const { account } = await algoIndexer
        .lookupAccountByID(props.accountAddress ?? "")
        .do();

      setAlgoAmount(account.amount / 10 ** 6);
      console.log("Balance of account " + JSON.stringify(theme.accountAddress));
      ...
      let nfts = _.sumBy(ownAssets, function (asset) {
        return asset.amount;
      });
      setNftAmount(nfts);
    };
